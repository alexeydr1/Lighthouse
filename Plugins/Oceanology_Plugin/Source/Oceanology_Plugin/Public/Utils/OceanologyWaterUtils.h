...
struct FOceanologyWaterUtils
{
	 ...
	 * @param InMaterialInterface is the parent material interface of the desired MID: can be a UMaterial or UMaterialInstanceConstant, in which case 
		...
		InMID is read-only so it's up to the caller to decide to update or not the MID. Usually, the calling code will be : MID = FOceanologyWaterUtils::GetOrCreateTransientMID(MID, ...)
	* @param InAdditionalObjectFlags is the additional flags that should be set on the newly-created object, if any (it will come out with at least RF_Transient as the name of the function implies)
	 ...
	 */
	static OCEANOLOGY_PLUGIN_API UMaterialInstanceDynamic* GetOrCreateTransientMID(
	...
	static OCEANOLOGY_PLUGIN_API EObjectFlags GetTransientMIDFlags()
	{
	 ...
	 *  Creates a transient render target of the proper size/format/... or returns the existing one if it's compatible.
	     InRenderTarget is read-only so it's up to the caller to decide to update or not the RT. Usually, the calling code will be : RT = FOceanologyWaterUtils::GetOrCreateTransientRenderTarget2D(RT, ...)
	 ...
	 */
	static OCEANOLOGY_PLUGIN_API UTextureRenderTarget2D* GetOrCreateTransientRenderTarget2D(
	...
	static OCEANOLOGY_PLUGIN_API bool IsWaterEnabled(bool bIsRenderThread);
	static OCEANOLOGY_PLUGIN_API bool IsWaterMeshEnabled(bool bIsRenderThread);
	static OCEANOLOGY_PLUGIN_API bool IsWaterMeshRenderingEnabled(bool bIsRenderThread);
	static OCEANOLOGY_PLUGIN_API float GetWaterMaxFlowVelocity(bool bIsRenderThread);
