...
struct FOceanologyWaterBodyRenderData
{
	...
	bool operator==(const FOceanologyWaterBodyRenderData& Other) const
	{
...
struct FOceanologyWaterQuadTree
{
	...
	struct FOceanologyTraversalOutput
	{
		 ...
		 */
		TArray<FOceanologyStagingInstanceData> StagingInstanceData;
	...
	/** Output of the quadtree when asking to traverse it for visible water tiles */
	struct FOceanologyTraversalDesc
	{
		int32 LowestLOD = 0;
		int32 LODCount = 0;
	...
	void AddFarMesh(const UMaterialInterface* InFarMeshMaterial, double InFarDistanceMeshExtent,
	...
	uint32 AddWaterBodyRenderData(const FOceanologyWaterBodyRenderData& InWaterBodyRenderData)
	{
		return NodeData.WaterBodyRenderData.Add(InWaterBodyRenderData);
	}

	/** Get bounds of the root node if there is one, otherwise some default box */
	FBox GetBounds() const
	{
		return NodeData.Nodes.Num() > 0 ? NodeData.Nodes[0].Bounds : FBox(-FVector::OneVector, FVector::OneVector);
	}
	...
	void BuildWaterTileInstanceData(const FOceanologyTraversalDesc& InTraversalDesc,
	...
	{
		return NodeData.GetAllocatedSize() + WaterMaterials.GetAllocatedSize() + FarMeshData.GetAllocatedSize();
	}
...
private:
	struct FOceanologyNodeData;

	/** Private recursion function, see AddOcean(...) */
	void AddOceanRecursive(const TArray<FVector2D>& InPoly, const FBox2D& InBox, const FVector2D& InZBounds,
	                       bool HSplit, int32 InDepth, uint32 InWaterBodyIndex);

	/** Private recursion function, see AddLake(...) */
	void AddLakeRecursive(const TArray<FVector2D>& InPoly, const FBox2D& InBox, const FVector2D& InZBounds, bool HSplit,
	                      int32 InDepth, uint32 InWaterBodyIndex);

	struct FOceanologyNode
	{
		FOceanologyNode() : WaterBodyIndex(0), TransitionWaterBodyIndex(0), ParentIndex(INVALID_PARENT),
		                    HasCompleteSubtree(1), IsSubtreeSameWaterBody(1), HasMaterial(0)
		{
		}

		/** If this node is allowed to be rendered, it means it can be rendered in place of all leaf nodes in its subtree. */
		bool CanRender(int32 InDensityLevel, int32 InForceCollapseDensityLevel,
		               const FOceanologyWaterBodyRenderData& InWaterBodyRenderData) const;

		/** Add instance for rendering this node*/
		void AddNodeForRender(const FOceanologyNodeData& InNodeData,
		                      const FOceanologyWaterBodyRenderData& InWaterBodyRenderData, int32 InDensityLevel,
		                      int32 InLODLevel, const FOceanologyTraversalDesc& InTraversalDesc,
		                      FOceanologyTraversalOutput& Output) const;
		...
		void SelectLODRefinement(const FOceanologyNodeData& InNodeData, int32 InDensityLevel, int32 InLODLevel,
		                         const FOceanologyTraversalDesc& InTraversalDesc,
		                         FOceanologyTraversalOutput& Output) const;

		/** Recursive function to select nodes visible from the current point of view */
		void SelectLOD(const FOceanologyNodeData& InNodeData, int32 InLODLevel,
		               const FOceanologyTraversalDesc& InTraversalDesc, FOceanologyTraversalOutput& Output) const;

		/** Recursive function to select nodes visible from the current point of view within an active bounding box */
		void SelectLODWithinBounds(const FOceanologyNodeData& InNodeData, int32 InLODLevel,
		                           const FOceanologyTraversalDesc& InTraversalDesc,
		                           FOceanologyTraversalOutput& Output) const;
		...
		bool QueryBaseHeightAtLocation(const FOceanologyNodeData& InNodeData, const FVector2D& InWorldLocationXY,
		                               float& OutHeight) const;

		/** Recursive function to query the bounds of a tile at a given location, return false if no leaf node could be found */
		bool QueryBoundsAtLocation(const FOceanologyNodeData& InNodeData, const FVector2D& InWorldLocationXY,
		                           FBox& OutHeight) const;

		/** Add nodes that intersect InMeshBounds. LODLevel is the current level. This is the only method used to generate the tree */
		void AddNodes(FOceanologyNodeData& InNodeData, const FBox& InMeshBounds, const FBox& InWaterBodyBounds,
		              uint32 InWaterBodyIndex, int32 InLODLevel, uint32 InParentIndex);

		/** Check if all conditions are met to potentially allow this and another node to render as one */
		bool CanMerge(const FOceanologyNode& Other) const
		{
			return Other.WaterBodyIndex == WaterBodyIndex && Other.TransitionWaterBodyIndex == TransitionWaterBodyIndex;
		}
		...
		/** Cached value to avoid having to visit this node's FOceanologyWaterBodyRenderData */
	...
	struct FOceanologyNodeData
	{
		...
		TArray<FOceanologyNode> Nodes;

		/** Render data for all water bodies in this tree, indexed by the nodes */
		TArray<FOceanologyWaterBodyRenderData> WaterBodyRenderData;
	...
	struct FOceanologyFarMeshData
	{
		struct FOceanologyFarMeshInstanceData
		{
		...
		TArray<FOceanologyFarMeshInstanceData> InstanceData;
