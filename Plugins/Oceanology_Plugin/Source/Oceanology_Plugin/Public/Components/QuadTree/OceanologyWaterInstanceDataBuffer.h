...
class TOceanologyWaterInstanceDataBuffers
{
public:
	static constexpr int32 NumBuffers = bWithWaterSelectionSupport ? 3 : 2;

	TOceanologyWaterInstanceDataBuffers(int32 InInstanceCount)
	{
	...
	~TOceanologyWaterInstanceDataBuffers()
	{
		for (int32 i = 0; i < NumBuffers; ++i)
		{
...
private:
	TArrayView<FVector4f> Lock(FRHICommandListBase& RHICmdList, int32 InInstanceCount, int32 InBufferID)
	{
		...
		FVector4f* Data = reinterpret_cast<FVector4f*>(RHICmdList.LockBuffer(
			Buffer[InBufferID], 0, SizeInBytes, RLM_WriteOnly));
